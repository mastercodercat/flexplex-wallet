...
export default function SendModalPage1(){
    ...
    const filteredAssets =
      query === ""
        ? assets
        : assets.filter((asset) => {
            return asset.assetName.toLowerCase().includes(query.toLowerCase()) || asset.assetSymbol.toLowerCase().includes(query.toLowerCase());
          });
    ...
    return (
      <>
          ...
          <Combobox value={selectedAsset} onChange={setSelectedAsset}>
            <div className="relative w-full col-span-12">
              <Combobox.Input
                ...
                displayValue={(asset: any) => asset ? `${asset.assetName} (${asset.assetSymbol})` : ''}
                        ...
                        {({ selected, active }) => (
                          <>
                            <div className='card cursor-pointer flex w-full'>
                              ...
                              <h1 className='text-right w-auto'><span className='text-base font-normal text-[#FAFAFA]'>{asset.assetAmount}</span>                     <br />
        ...
        <div className="bg-[#1e1e1e] border border-[#383838] rounded-xl h-24">
          <input
            title="Value"
            value={selectedAsset ? `${selectedAsset.assetAmount} ${selectedAsset.assetSymbol}` : ``}
            type="text"
