    ...
    const filteredAccounts =
      query === ""
        ...
        : accounts.filter((account) => {
            return account.accountName.toLowerCase().includes(query.toLowerCase()) || account.accountSymbol.toLowerCase().includes(query.toLowerCase());
          });
                            ...
                            >
                                <Dialog.Panel className="w-full max-w-[30rem] transform overflow-hidden rounded-xl bg-[#1e1e1e] border border-[#383838] text-left align-middle shadow-xl ring-1 ring-black ring-opacity-5 focus:outline-none transition-all">
                                    ...
                                    <div className='min-h-[32rem] mb-5 py-5 px-9'>
                                        <h1 className='text-sm mb-3 font-light'>Select an Account</h1>
                                        <div className="grid grid-cols-7 bg-[#1e1e1e] border border-[#383838] rounded-xl">
                                            <Combobox value={selectedAccount} onChange={setSelectedAccount}>
                                                ...
                                                <Combobox.Input
                                                    ...
                                                    displayValue={(account: any) => account ? `${account.accountName} (${account.accountSymbol})` : ''}
                                                    ...
                                                    {filteredAccounts.length === 0 && query !== "" ? (
                                                        ...
                                                        >
                                                            ...
                                                            <>
                                                                ...
                                                                <img src={account.accountImage} alt="" className="w-12 h-12 rounded-full" />
                                                                <h1 className='ml-2'><span className='text-md font-normal '>{account.accountName}</span>                     <br />
                                                                    <span className='opacity-40 font-thin text-sm'>{account.accountSymbol}</span></h1>
                                                                    <div className='flex-grow'></div>
                                                                    <h1 className='text-right w-auto'><span className='text-base font-normal text-[#FAFAFA]'>{account.accountAmount}</span>                     <br />
                                                                    <span className='opacity-40 font-normal text-sm'>Available</span>
                                                                </h1>
                                                                </div>
                                                            </>
                                                            )}
                                        ...
                                        <div className='grid grid-cols-7 bg-[#1e1e1e] border border-[#383838] rounded-xl mb-2'>
                                            <input placeholder='NA' type='text' value={selectedAccount.accountAddress} className='col-span-6 focus:border-transparent font-light focus:ring-0 rounded-xl bg-[#1e1e1e] border-0 text-[#ffffff] text-left text-xs py-3'></input>
                                    ...
                                    <div className='col-span-2 pb-8 px-6 w-full flex justify-center'>
                                        <button onClick={() => setIsOpen(false)} className="focus:border-transparent focus:ring-0 w-64 text-black bg-[#02ff63] transition ease-out hover:-translate-y-1 hover:scale-100 delay-150 hover:shadow-lg hover:shadow-[#02ff63cc] font-bold rounded-xl text-md p-2 ml-2 mt-3" type="submit">Close</button>
                                    </div>
                                </Dialog.Panel>
                            </Transition.Child>
                        </div>
