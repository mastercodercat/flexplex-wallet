...
function NetworkCard({network, rpcName, rpcEndpoint, currRpcEndpoint, setRpcEndpoint}: {network: Cluster, rpcName: string, rpcEndpoint: string, currRpcEndpoint: string, setRpcEndpoint: (rpcEndpoint: string) => void}) {
    return (
        <div className='col-span-2 grid grid-cols-4 bg-[#232323] border border-[#383838] rounded-xl w-100 mb-4'>
            <div className='col-span-3'>
                <h1 className='text-white text-left text-xs pt-4 pb-1 pl-6'>{rpcName}</h1>
                <h1 className='text-[#ffffff66] text-left text-xs pb-4 pl-6'>{rpcEndpoint}</h1>
            </div>
            <div className='flex justify-end'>
                <button onClick={() => setRpcEndpoint(network)} type='button' title="Revoke" className='bg-[#272727] p-3 px-6 w-[4.2rem] rounded-xl'>
                    {rpcEndpoint === currRpcEndpoint &&
                        <CheckCircleIcon />
                    }
                </button>
            </div>
                                ...
                                <Dialog.Panel className="w-full max-w-[30rem] transform overflow-hidden rounded-xl bg-[#1e1e1e] border border-[#383838] text-left align-middle shadow-xl ring-1 ring-black ring-opacity-5 focus:outline-none transition-all">
                                    ...
                                    <div className='mb-5 py-3 px-0'>
                                        ...
                                        <div className='col-span-2 pb-8 w-full flex justify-center'>
                                            <button onClick={() => setIsOpen(false)} className="focus:border-transparent focus:ring-0 w-64 text-white bg-[#272727] border-[#383838] border transition ease-out hover:-translate-y-1 hover:scale-100 delay-150 hover:shadow-lg font-bold rounded-xl text-sm py-3 px-3 mt-3" type="submit">Done</button>
                                        </div>
